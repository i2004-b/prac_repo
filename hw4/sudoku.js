...
var printBoard = function () { 
    // Double for loop to iterate through the 2-D array.  
    for (var row = 0; row < 9; row++) {
        for (var col = 0; col < 9; col++) {
            // cellId contains the content needed to placed.
            var cellId = "r" + (row + 1) + (col + 1);
            document.getElementById(cellId).innerHTML = sol[row][col]; // innerHTML 
        }
    }
};
...
var loadNewPuzzle = function() {
    ...
    request.onload = function() {
        ...
        // Update the DOM with the new puzzle.
        printBoard();
    }
    request.send();
};
